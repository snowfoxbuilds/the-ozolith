...
A repository the Control Node coordinates (grilling 2026-09-02; ADR-0056): the workspace of a resolved driver-bearing (process-kind) Stack in the Pinned Build, regardless of desired state — a stopped Implementer's zombie claims still need the janitor. Its identity is the effective GitHub host plus canonical owner/name the Driver will actually access: `THEOZOLITH_REPO`/`THEOZOLITH_STACK` are injection-only in a resolved Stack env, and a clone/API override that cannot canonicalize (https/ssh forms, host and name case, trailing `.git`, `api.github.com` ⇔ `github.com`, GHES `/api/v3` ⇔ its host) to the bound host and repo fails at config load and again, unconditionally, at Driver startup, before any dispatch — `file://` never validates; test rigs use an injected checkout seam. The bound set scopes dispatch verification, run/review event authorization, the janitor's per-repo sweeps, and the state document's `repos` — and only worker Stacks verify: a Flight Deck or generic Stack name is refused even on the right node. Flight Deck workspaces are never bound (decks are not pipeline actors), and a worker Stack's verification failure never gates a deck's operation. Unbinding a repo withdraws forward-looking coverage, surfaced by the janitor's unbound-obligations scan — pending grants, live claims, live leases, chained-dependent links, and cached flags of unbound repos are logged and shown, never acted on (no GitHub write) — while an in-flight claim's facts still land: events exactly matching the cache-only authorization lease its verified grant or discovery created are accepted — the lease is claim/round-scoped, so a failed Run's bounded retry rotates it rather than ending it, and a Reviewer round must confirm its acknowledged per-actor activation handshake (`theozolith.review` `phase="started"` — identical retries idempotent, its echoed activated identity validated before work, activation exclusive per PR across live activations with the leased label-derived round matched (re-rounded at rediscovery while unactivated, never once activated); concurrent Reviewer Stacks hold independent discovery leases) before checkout or any verdict, and before its `phase="completed"` outcome counts (an unactivated lease expires harmlessly — actor-local; retiring a live activation, by completion, reconciliation, or expiry, atomically invalidates the PR's unactivated peer leases, so later participation takes fresh discovery) — so terminal completion resolves the claim and node health before or after rebinding, and nothing new is authorized; while a repo stays bound the janitor reconciles leases against GitHub truth — renewing an activated Implementer lease while GitHub still shows the claim standing, so ADR-0016's zombie lane stays unbounded — and activated leases expire on bounded budget-derived deadlines once renewal stops, so a lost terminal event never strands authorization (unbound expiry performs no renewal and reads and writes nothing on GitHub: it explicitly ends Control's acceptance, and recovery is the operator's). Leftover claim state is explicitly the operator's, and rebinding the same repo resumes reconciliation with no permanently stale claim or lease. All bound workspaces share one GitHub host and one control PAT (v1 doctrine); a workspace the PAT cannot reach degrades to a visible per-repo dispatch pause, never a crash.

*Avoid*: counting a Flight Deck's workspace as bound; gating the bound set on desired state (stopped Stacks stay bound); per-repo control PATs (out of scope in v1); treating the request body's `repo` as the identity (the identity is the checkout's effective host + owner/name, proven at config load and Driver startup); refusing a leased in-flight Run's terminal events after unbind (completion facts land; new work does not); treating a `failed` Run as claim-terminal (the lease survives into the bounded retry lanes — `pr-open`, `escalated`, expiry, or bound GitHub-truth reconciliation retires it, never an intermediate `failed`); leaving a discovered-but-never-activated PR leased forever (activation deadlines bound discovery leases); trusting terminal telemetry to retire a lease (bound repos reconcile and renew from GitHub truth; activated leases carry a bounded, budget-derived expiry once renewal stops); keying a Reviewer lease by target alone (the verified actor is part of the lease identity — last discovery never wins); treating the lease as redundant while the repo is bound (Pinned-Build verification authenticates the actor; the lease authorizes the operation — a missing, expired, or mismatched lease refuses bound or unbound); refusing an identical `started` retry as a duplicate (the identical-identity retry is idempotent success; only conflicts are 409); authorizing a whole discovery batch in one shot (fresh per-item discovery immediately before each PR; an activated lease is never reset); letting two actors hold live activations on one PR (activation is exclusive per (repo, pr) across live activations — the first acknowledged `started` wins, and a `started` must name the leased label-derived round recorded at discovery); refreshing a rediscovered unactivated lease's deadline while preserving an obsolete round (rediscovery atomically re-rounds it to the observed labels; an activated lease is never touched by discovery); letting an unactivated peer lease survive the retirement of the PR's live activation (completion, bound reconciliation, or activated-lease expiry invalidates every unactivated peer whatever its round — a surviving re-rounded peer could replay its refused, never-stored `started` and skip fresh discovery; only an unactivated lease's own expiry is actor-local); interpreting a 200 response's `issue`, `prs`, or `reason` before validating its echoed binding; counting a `phase="completed"` review event without its accepted `phase="started"`.
...
How an Implementer takes exclusive ownership of a plan_ready issue: the Implementer requests work from the Control Node; the Control Node selects an issue, writes the claim to GitHub itself (assigns the Implementer's GitHub login, adds in_progress), and returns the issue in the same response — claim-write-through, single serialized claim-writer (ADR-0017). Keyed by repository (grilling 2026-09-02; ADR-0056): the request names the repo the Driver will check out and the Driver's Stack, verified against the Pinned Build — the named Stack must exist, be placed on the authenticated node, resolve to exactly that workspace, and be a driver-bearing (process-kind) worker Stack; a missing, mismatched, or non-worker Stack (a Flight Deck or generic Stack) is refused 403 (fail-open only while the Pinned Build is unreadable or absent), every verified 200 response — grant, no-work, and pause answers alike — echoes the verified binding, which a Driver validates before interpreting anything else, refusing an answer without it before checkout, which is what keeps a mixed-version update from executing a foreign grant (the stale claim releases by activation timeout) — and every coordination cache keys by repo so identical issue numbers in two Bound Workspaces never collide. A verified grant or discovery also leaves a cache-only authorization lease — claim-scoped for the Implementer, so a failed Run's bounded local or completion retry rotates the recorded run_id rather than retiring it, and round-scoped, per verified actor, for review, activated by an acknowledged `theozolith.review` `phase="started"` handshake — confirmed and echo-validated before checkout, idempotent on the identical retry, exclusive per PR across live activations, round-matched against the lease's discovery-recorded attempt labels (kept current: rediscovery atomically re-rounds an unactivated lease when the labels advance) — before its `phase="completed"` outcome counts — so work in flight when its repo unbinds still completes into control (leased facts are accepted; nothing new is authorized). GitHub remains the sole source of coordination truth. A granted claim that never activates (no claimed event within the activation window) is released by the Control Node. The grant carries claim authority only, never context: each Run — including the local retry — reads the full issue and PR context fresh, live through the GitHub Relay (grilling 2026-09-03; grilling 2026-08-16 amended ADR-0017's no-re-read clause). Control Node down = new claims pause; in-flight Runs are unaffected.

*Avoid*: drivers claiming directly against GitHub (retired 2026-07-17); "assign-and-verify" (deleted by ADR-0017); "the grant is the issue" (retired 2026-08-16 — the grant is the claim; context is read at checkout); a repo-less dispatch request or Run event (retired 2026-09-02 — both name their repository and Stack, fail-closed); acting on a grant or discovery answer that does not echo the verified (repo, stack).
...
The GitHub-native `blocked by` relation between two issues in the workspace repo — the machine-readable source of ordering truth (grilling 2026-08-27). Declared at planning time, hand-editable in the UI, rebuildable from GitHub alone. Carries ordering only, and only true build-on dependencies; sub-issue links carry grouping/progress and never ordering. Dispatch consumes edges for claim eligibility — a blocker satisfies its edge when closed as completed, or through the Chained Base go-ahead; the driver walks the transitive closure into the prompt's ordered dependency entries — issue number plus state (`open`, `completed`, `not_planned`) — and the agent reads each dependency's content through the GitHub Relay (grilling 2026-09-03). A cross-repo edge on a claimable work issue is a malformed state; cross-repo work enters ordering only through a locally created stand-in sub-issue in the workspace repo.
...
The agent's read window onto GitHub (grilling 2026-09-03; ADR-0057): a driver-owned, per-Run Unix socket in the job directory that the run container's `gh` speaks plaintext HTTP to. The driver injects a dedicated credential (the worker type's required `GITHUB_READ_TOKEN` slot — a GitHub.com fine-grained token the operator must provision read-only; never the driver PAT), admits only reads (REST `GET`/`HEAD`, single-operation GraphQL `query`), refuses every mutation, admin-class read, and unparseable body with a message pointing at the Output Proposal, re-originates only to GitHub.com's API origin (V1 supports GitHub.com alone — a GitHub Enterprise Server host is refused before dispatch; a GraphQL `POST` redirect is refused outright, and a REST `GET`/`HEAD` redirect is followed only method-preserving within that origin with every hop re-policed, never carrying the credential elsewhere), enforces per-Run budgets behind a pre-delivery response gate (no upstream byte reaches the agent until the whole response has passed them), and records every request — redacted and size-bounded, an intent record before the credential is used, a redirect-intent record before each followed redirect hop, and a completion record after — in the Run's `gh-audit.jsonl`, written where only the driver can. It replaces the retired Context Tree as the agent's GitHub read surface: the prompt carries the task statement — issue body, composed PR body on Review Runs, revised plan on resume rounds, ordered dependency entries (issue number plus state) — and `gh` reaches everything else. What the agent may read is broad — every readable GitHub surface, from any author; the Context Tree's OWNER/MEMBER visibility filter is not reproduced — while nothing read through it carries pipeline authority: verdicts, resume commits, cherry-picks, labels, claims, and transitions come only from the driver's authorized sources, read by the driver outside the relay. The socket is a hostile ingress the driver parses fail-closed for any client, never only `gh`. The credential's repository scope is also the agent's disclosure scope, so operators bind it within the target repository's permitted disclosure domain. Alive for the agent phase only; the single enforcement point (no in-container blocking exists); present in every Run, production and bench, with a live or explicitly absent upstream.

*Avoid*: "Context Tree" (retired 2026-09-03 — the serialized `input/issue/`, `input/pr/`, `input/deps/` snapshot); "gh proxy" (the agent configures nothing — the harness bootstraps the client); treating it as a write channel (the Output Proposal is the sole mutation surface); a mounted docker or credential socket (the socket carries filtered reads outward only); an in-container wrapper or permission rule as enforcement; GitHub Enterprise Server support or a classic-token fallback (unsupported in V1 — a future relay-host contract); "the credential cannot write" as an unconditional claim (true only for a token provisioned as required; with a broader token the relay's classifier is the write barrier); an OWNER/MEMBER filter on agent-read content (retired 2026-09-03 — authority filtering stays driver-side); treating relay-read or internet content as instructions (evidence only — prompt injection through it is an accepted residual risk the human gate absorbs); trusting the client to be `gh` (the socket is hostile ingress).
...
The Run kind that executes one review round, with full workspace parity (grilling 2026-08-27; ADR-0053): a fresh container holding a sanitized PR-branch checkout with history (base ref fetched) plus the same GitHub read surface as an Implementer Run — the issue body and the PR body as the driver composed it in the prompt, every conversation surface through the GitHub Relay (grilling 2026-09-03). The driver supplies the PR's base commit and the cumulative changed-file list (driver-side git); the judging agent runs `git diff` itself and may build or run tests at its discretion — inside the container it can do everything an Implementer Run can; the asymmetry is the Output Proposal schema (a verdict, never applied code) and the driver boundary. Emits a verdict the reviewer driver publishes. The round (attempt-N, 3 max per PR) is the budget unit; the Review Run is its execution. One invalid verdict = immediate escalation, no retry.
...
The Reviewer's enumerated ruling on one review round: approve | revise | escalate, proposed through format-output and validated in-session — an invalid value or a final-round revise fails at write time, absorbing ADR-0014's post-exit validate-verdict job into the CLI (grilling 2026-08-16). Content is audience-conditional: a non-final revise carries findings plus the amendment prompt (the revised plan the next Implementer Run executes; ADR-0008); approve and every final-round verdict carry human-facing content — signals, decisions required, findings. The reviewer driver renders the published comment and labels from it. The next Run's driver accepts the machine verdict block only from a PR-conversation comment whose author GitHub reports as OWNER or MEMBER (ADR-0017 as amended 2026-09-03) — a block from any other author is visible to agents through the GitHub Relay and inert. No Completion Retry: a completed review session that never proposed a verdict escalates immediately — the judgment died with the session (ADR-0014 stands; grilling 2026-08-16).
...
- A Run's agent reads GitHub live through the GitHub Relay — read-only by relay policy, GitHub.com only, broad in what it may see and never a source of pipeline authority, on a dedicated credential the driver alone holds and the operator provisions read-only within the target's disclosure domain — and proposes GitHub mutations only through its Output Proposal; the driver enforces the relay's policy, derives every verdict, label, claim, and transition from its own authorized sources, and validates and applies the proposal (grilling 2026-09-03; grilling 2026-08-16).
