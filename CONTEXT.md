...
The self-contained export of one worker-type definition for benchmarking (grilling 2026-08-28; ADR-0054): a docker-buildable directory carrying the definition's resolved manifest (fields, pins, adapter, instruction hash, non-identity metadata), the compiled knowledge tree for the candidate's adapter tool, and a generated Dockerfile — producing the same derived image a deployment would run. Exported by `theozolith candidate export` from any config-repo-shaped source tree, never from the Pinned Build; equivalence with a deployed image is identity-hash equality, recomputed by consumers from bundle bytes. Secret slot names travel; secret values never do. A candidate is kind-agnostic — implementer vs reviewer is a benchmark-mode property, never part of the bundle or its identity.

*Avoid*: "candidate" for the artifact (the candidate is the worker-type definition; the bundle is its portable export); trusting a recorded identity (consumers recompute); vendoring the raw knowledge source (the bundle carries the compiled per-tool tree the knowledge pin actually covers).
...
- Every Run's checkout is a self-contained reference clone (git clone --reference --dissociate) from a driver-owned node-local mirror that is never mounted into containers (grilling 2026-08-16); driver-side git runs maintenance-free and deterministic, and a failed reference clone falls back to one full network clone — the mirror is an optimization, never load-bearing for Run success (grilling 2026-08-28).
