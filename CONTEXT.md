...
A repository the Control Node coordinates (grilling 2026-09-02; ADR-0055): the workspace of a resolved driver-bearing (process-kind) Stack in the Pinned Build, regardless of desired state — a stopped Implementer's zombie claims still need the janitor. The bound set scopes dispatch verification, the janitor's per-repo sweeps, and the state document's `repos`. Flight Deck workspaces are never bound (decks are not pipeline actors), and a worker Stack's verification failure never gates a deck's operation. Unbinding a repo withdraws coordination coverage: leftover claims are the operator's — the janitor logs live claims in unbound repos and never writes GitHub for them. All bound workspaces share one GitHub host and one control PAT (v1 doctrine); a workspace the PAT cannot reach degrades to a visible per-repo dispatch pause, never a crash.

*Avoid*: counting a Flight Deck's workspace as bound; gating the bound set on desired state (stopped Stacks stay bound); per-repo control PATs (out of scope in v1).
...
How an Implementer takes exclusive ownership of a plan_ready issue: the Implementer requests work from the Control Node; the Control Node selects an issue, writes the claim to GitHub itself (assigns the Implementer's GitHub login, adds in_progress), and returns the issue in the same response — claim-write-through, single serialized claim-writer (ADR-0017). Keyed by repository (grilling 2026-09-02; ADR-0055): the request names the repo the Driver will check out and the Driver's Stack, verified against the Pinned Build — the named Stack must exist, be placed on the authenticated node, and resolve to exactly that workspace; a missing or mismatched Stack is refused 403 (fail-open only while the Pinned Build is unreadable or absent), and every coordination cache keys on (repo, issue) so identical issue numbers in two Bound Workspaces never collide. GitHub remains the sole source of coordination truth. A granted claim that never activates (no claimed event within the activation window) is released by the Control Node. The grant carries claim authority only, never context: each Run — including the local retry — re-reads the full issue and PR context fresh at checkout (grilling 2026-08-16, amending ADR-0017's no-re-read clause). Control Node down = new claims pause; in-flight Runs are unaffected.

*Avoid*: drivers claiming directly against GitHub (retired 2026-07-17); "assign-and-verify" (deleted by ADR-0017); "the grant is the issue" (retired 2026-08-16 — the grant is the claim; context is read at checkout); a repo-less dispatch request or Run event (retired 2026-09-02 — both name their repository, fail-closed).
...
The product's central service, shipped in TheOzolith's control/ component (deployed on the Pi). Renders the fleet dashboard; receives Node Daemon heartbeats and Run events; answers heartbeats with infrastructure commands (drain, recycle, update, rebuild); dispatches claims as the single writer of claim creation on GitHub across every Bound Workspace of its deployment (ADR-0017 as amended by ADR-0055; keyed by repository, grilling 2026-09-02); escalates zombie claims evidence-first and quarantines failing nodes at dispatch (ADR-0016). Authoritative for node and docker lifecycle and for claim dispatch; never originates other coordination — it cannot approve, revise, or advance an issue, and GitHub remains the sole source of coordination truth. Control Node down: in-flight Runs finish and publish; new claims and review rounds pause.

*Avoid*: treating its database as coordination truth (GitHub is); "advisory" for the claim path (retired 2026-07-17; ADR-0017); `THEOZOLITH_REPO` as a Control Node setting (retired 2026-09-02 — the repo set is what the Pinned Build's driver-bearing Stacks bind).
...
- The Control Node dispatches claims and review rounds, observes workers and Runs, and writes claim creation to GitHub; GitHub owns all coordination state (ADR-0017). Coordination is keyed by repository: one Control Node coordinates every Bound Workspace of its deployment, over one GitHub host and one control PAT (grilling 2026-09-02; ADR-0055).
