...
The agent's read window onto GitHub (grilling 2026-09-03; ADR-0057): a driver-owned, per-Run Unix socket in the job directory that the run container's `gh` speaks plaintext HTTP to. The driver injects a dedicated read-only credential (the worker type's required `GITHUB_READ_TOKEN` slot — never the driver PAT), admits only reads (REST `GET`/`HEAD`, single-operation GraphQL `query`), refuses every mutation, admin-class read, and unparseable body with a message pointing at the Output Proposal, re-originates only to the Bound Workspace's host (following a redirect only within that origin, never carrying the credential elsewhere), enforces per-Run budgets, and records every request — redacted and size-bounded — in the Run's `gh-audit.jsonl`, written where only the driver can. It replaces the retired Context Tree as the agent's GitHub read surface: the prompt carries the task statement — issue body, composed PR body on Review Runs, revised plan on resume rounds, ordered dependency issue numbers — and `gh` reaches everything else. Alive for the agent phase only; the single enforcement point (no in-container blocking exists); present in every Run, production and bench, with a live or explicitly absent upstream.

*Avoid*: "Context Tree" (retired 2026-09-03 — the serialized `input/issue/`, `input/pr/`, `input/deps/` snapshot); "gh proxy" (the agent configures nothing — the harness bootstraps the client); treating it as a write channel (the Output Proposal is the sole mutation surface); a mounted docker or credential socket (the socket carries filtered reads outward only); an in-container wrapper or permission rule as enforcement.
...
- A Run's agent reads GitHub live through the GitHub Relay — read-only, on a dedicated credential the driver alone holds — and proposes GitHub mutations only through its Output Proposal; the driver enforces the relay's policy and validates and applies the proposal (grilling 2026-09-03; grilling 2026-08-16).
