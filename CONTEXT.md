...
A repository the Control Node coordinates (grilling 2026-09-02; ADR-0056): the workspace of a resolved driver-bearing (process-kind) Stack in the Pinned Build, regardless of desired state — a stopped Implementer's zombie claims still need the janitor. Its identity is the effective GitHub host plus canonical owner/name the Driver will actually access: `THEOZOLITH_REPO`/`THEOZOLITH_STACK` are injection-only in a resolved Stack env, and a clone/API override that cannot be proven to address the bound host and repo fails at config load and again at Driver startup, before any dispatch. The bound set scopes dispatch verification, run/review event authorization, the janitor's per-repo sweeps, and the state document's `repos`. Flight Deck workspaces are never bound (decks are not pipeline actors), and a worker Stack's verification failure never gates a deck's operation. Unbinding a repo withdraws coordination coverage, surfaced by the janitor's unbound-obligations scan — pending grants, live claims, and cached flags of unbound repos are logged and shown, never acted on (no GitHub write); leftover claim state is explicitly the operator's, and rebinding the same repo resumes reconciliation unchanged. All bound workspaces share one GitHub host and one control PAT (v1 doctrine); a workspace the PAT cannot reach degrades to a visible per-repo dispatch pause, never a crash.
