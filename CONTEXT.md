...
- A Run's agent reads GitHub live through the GitHub Relay — read-only by relay policy, GitHub.com only, on a dedicated credential the driver alone holds and the operator provisions read-only — and proposes GitHub mutations only through its Output Proposal; the driver enforces the relay's policy and validates and applies the proposal (grilling 2026-09-03; grilling 2026-08-16).
