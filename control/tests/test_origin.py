...
def test_compose_write_read_roundtrip(tmp_path):
    ...
    path = origin.write_public_origin(tmp_path, text)
    assert path.read_text().strip() == text
    assert origin.read_public_origin(tmp_path) == text
...
def test_origin_init_is_persistent_until_forced(cli_env):
    ...
    first = origin.read_public_origin(cli_env)
    ...
    assert origin.read_public_origin(cli_env) == first

    assert cli_main(["origin-init", "--force", "--base-domain", "theozolith.com"]) == 0
    second = origin.read_public_origin(cli_env)
...
def test_origin_init_can_pin_a_nonstandard_external_port(cli_env):
    ...
    text = origin.read_public_origin(cli_env)
    ...
    assert ":" not in origin.read_public_origin(cli_env).removeprefix("https://")
...
def test_tls_init_includes_the_hostname_but_never_a_port(cli_env):
    ...
    hostname = origin.parse_public_origin(origin.read_public_origin(cli_env)).hostname
