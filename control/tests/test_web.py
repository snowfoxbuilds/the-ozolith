...
def test_secret_write_surfaces_reject_an_unsafe_stored_name(control: ControlRig):
    ...
    login(control)
    # The API path param — a name in the writer's '.<name>.tmp' temporary
    # namespace is reserved and never routes into the store.
    api = control.admin("PUT", "/api/v1/secrets/.token.tmp", body={"value": "v"})
