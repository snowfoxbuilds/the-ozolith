...
- The canonical browser origin is `https://<control_ip>[:<control_port>]` — the same persisted address nodes dial (ADR-0031). `control_port` is a new read-only `[control]` field beside `control_ip`, default 443; `init --port` sets a nonstandard one for shared boxes.
