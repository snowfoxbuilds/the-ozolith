...
- The `ExecStart` path is validated as reachable by the service user before the unit is written (others-execute on the file and every ancestor directory): a sudo invocation happily resolves an executable inside `/root` or a 0700 home venv, and persisting that would fail only at first boot. Setup fails with remediation ("install at a system path, e.g. a venv under /opt") instead of deferring the failure.
