...
- **Local retry**: on a failed Run the driver keeps the claim and re-launches once, locally. The retry is a full second Run — new run_id, fresh clone/worktree, fresh container, its own evidence bundle. Uniform budget: any non-completed Run burns the single retry regardless of class (timeout, session death, harness crash, zero commits without reasoning, pre-session infra failure). On a second non-completion the driver releases the claim and escalates, linking both evidence bundles and stating each failure's class. The run-failed marker machinery (marker comment, sanitizer, budget-check-at-claim) is deleted. Retry classes are kind behaviour: Issue Workers keep this local retry and the completion retry below; PR Workers get neither — their judgment dies with the session (amended 2026-09-03, #120).
- **failed label**: every no-outcome ending of any worker's Run applies failed + needs_human on the trigger object — the issue for an Issue Worker (driver: second local failure; janitor: zombie), the PR for a PR Worker (a missing or invalid outcome, no retry) — one failure lane for both kinds (amended 2026-09-03, #120; the review lane's blocked + needs_human for a garbled verdict is retired). blocked + needs_human is reserved for a decision the human owes: round exhaustion, an explicit escalate outcome, the janitor's base-drift lane. Two queryable flavors: failed = execution broke, autopsy the evidence; blocked = a decision is owed. Only the human removes failed, as part of re-queueing. failed is a Core Label and a product-fixed dispatch block: no worker of either kind is granted an issue:PR set whose issue or PR carries failed, whatever its Intake declares (amended 2026-09-03, #120 — the former failed + plan_ready malformed-state row is retired; the label itself is the human's queue).
...
- **2026-09-03 (#120, ADR-0057)**: one failure lane for both worker kinds. Under the declarative worker model every no-outcome ending — Issue Worker or PR Worker — lands `failed` + `needs_human` on the trigger object; the review lane's `blocked` + `needs_human` for a missing or invalid verdict is retired, and `blocked` is reserved for decisions the human owes (round exhaustion, an explicit escalate outcome, base drift). Retry classes are kind behaviour: Issue Workers keep the local retry and the completion retry ("Implementer-only" above now reads "Issue Workers only"); PR Workers get none. `failed` is a Core Label no configured output can write and a product-fixed dispatch block on either object of the issue:PR pair for every worker regardless of its declared Intake — the `failed` + `plan_ready` malformed-state row is retired because the label itself is the queue. Reason: a failure lane an operator's Intake could accidentally bypass is not a failure lane.
