...
`evict_progress` records eviction evidence in the same transaction as its deletes — one bounded row per **(type, node, component)** combination seen among the evicted rows (the extracted columns the read filters match against), each carrying the **lowest and highest evicted ids** (the range bounds that answer the strict cursor predicate), a running count, the last eviction time, and the newest evicted `received_at`. The evidence is capped (64 scopes): past the cap — far above the real cardinality of evictable types × fleet nodes × components — it collapses into a single `'*'` sentinel row that matches everything, the conservative direction. Evidence about evictions is never allowed to grow into an archive inside the cache (ADR-0016).

The response contract is explicit about the two questions a client can ask:

- **`evicted`** — *query-relative*: true exactly when an evicted row **could have matched this call's** `type`/`node`/`component` filters, its `since` window (`since <= that scope's newest evicted received_at`; ids and timestamps advance together under the store's one clock), **and its cursor bound** — a page showing `id < cursor` lost a row iff a matching scope's `first_evicted_id` sits below the bound (0 = unknown lower bound, from a pre-amendment row: conservative). Evicting the cursor-boundary event itself never marks the *next* page incomplete. This is the field consumers act on — `theozolith status` degrades exactly when **its own** `theozolith.error` query is incomplete (ADR-0039), and an unrelated progress eviction no longer produces a false verdict.
...
- **Row-level eviction tombstones** (record each evicted row's key columns): an archive of the evicted growing inside the cache — exactly what cache-not-archive forbids; the bounded scope rows answer the only question clients ask ("could my query have lost rows?") at fixed size.
