...
`evict_progress` records eviction evidence in the same transaction as its deletes — one bounded row per **(type, node, component)** combination seen among the evicted rows (the extracted columns the read filters match against), each carrying the highest evicted id, a running count, the last eviction time, and the newest evicted `received_at`. The evidence is capped (64 scopes): past the cap — far above the real cardinality of evictable types × fleet nodes × components — it collapses into a single `'*'` sentinel row that matches everything, the conservative direction. Evidence about evictions is never allowed to grow into an archive inside the cache (ADR-0016).

The response contract is explicit about the two questions a client can ask:

- **`evicted`** — *query-relative*: true exactly when an evicted row **could have matched this call's** `type`/`node`/`component` filters and `since` window (`since <= that scope's newest evicted received_at`; ids and timestamps advance together under the store's one clock). Cursor bounds never exclude: eviction is oldest-first, so evicted rows are precisely what backward pagination was heading toward. This is the field consumers act on — `theozolith status` degrades exactly when **its own** `theozolith.error` query is incomplete (ADR-0039), and an unrelated progress eviction no longer produces a false verdict.
