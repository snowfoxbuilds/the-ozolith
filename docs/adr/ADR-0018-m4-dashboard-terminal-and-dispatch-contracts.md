...
### Admin credential and session (superseded 2026-07-27, ADR-0023 — see Amendments)
...
- **2026-07-27 (grilling session) — "Separate dashboard password: rejected" is reversed** by ADR-0023: the original rejection weighed authority (unchanged — the password protects the same single-operator trust domain as the admin token), not ergonomics. Init-time password entry changed the ground: humans do not memorize 128-bit bearer tokens.

## Relevant PRs

- #5 — implementing PR for this ADR, under the M4 delegated-decisions mandate.
