...
5. **The driver–harness interface is a per-Run job directory.** input/ (prompt, checkout, review input files) and output/ (decisions or verdict file, transcript, status) (amended 2026-08-17, ADR-0046). No network channel and no in-container reporter: container liveness is observed by the Node Daemon via docker; semantic Run events are emitted by the driver; session state reaches the driver as files. One enumerated exception (amended 2026-09-03, ADR-0057): the job directory may hold the GitHub Relay's Unix socket file, through which the agent's `gh` obtains policy-filtered, read-only GitHub answers from a credential the driver alone holds — the socket carries no credential, authority, or reporter into the container, and the job directory stays the only channel.
...
- Credential isolation is enforced by a container boundary: no LLM ever shares a box with a PAT; the transition-authority matrix holds by construction. Web-terminal attach targets are live run containers only (reported via heartbeats) (amended 2026-07-21, ADR-0019); ssh-to-node + docker exec needs no in-container daemon.
...
- **2026-07-21 (ADR-0019)**: interactive tmux harness mechanics retired; Runs execute headless and are not attach targets (interactivity moves to the Flight Deck — then called Pilot).
- **2026-08-17 (ADR-0046)**: the job directory's output half carries the schema-validated Output Proposal (`output/proposal.json`, written through the format-output CLI) instead of a mode-specific decisions or verdict file; the driver validates and applies it post-exit as the sole policy boundary.
- **2026-09-03 (ADR-0057)**: decision 5 gains its enumerated exception — the job directory may carry the GitHub Relay socket file; `gh` in the container reads GitHub through a driver-held read-only credential. The reason the clause existed (no credential or reporter inside the LLM's box) is preserved: the socket carries filtered reads outward only.

## Relevant PRs

- #2 — original M2 implementation (long-lived Worker containers running headless agent subprocesses); retracted by this decision, its surviving content absorbed here.
