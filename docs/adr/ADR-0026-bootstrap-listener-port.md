...
Provenance: delegated decision from the M7 brief; implements ADR-0023's bootstrap-listener contract.

# ADR-0026: Bootstrap listener default port and process arrangement

## Context

ADR-0023 fixed the listener's shape — dedicated plaintext port, GET-only, closed route table of three inert values, never mounted on the HTTPS app — and delegated the default port and its systemd/socket arrangement.
