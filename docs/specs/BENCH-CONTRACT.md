...
Every Run — production and bench, both kinds — carries the GitHub Relay (grilling 2026-09-03): the driver-owned, per-Run Unix socket in the job dir through which the agent's `gh` reads GitHub with a credential the driver alone holds. The bench driver runs the product's relay through the exposed entry point below, never a copy, so policy is identical by construction: GitHub.com as the only upstream host (V1 supports GitHub.com alone — a GitHub Enterprise Server host is refused before any run starts), read-only (REST `GET`/`HEAD`, single-operation GraphQL `query`), fail-closed classification, the admin-read denylist, per-request and aggregate per-Run budgets enforced by a pre-delivery response gate (no byte of an upstream response reaches the agent until the whole response has passed them), redirect handling that refuses every 3xx to a GraphQL `POST` and follows a REST `GET`/`HEAD` redirect only method-preserving with every hop revalidated against the GitHub.com origin and policy — never forwarding the credential elsewhere — agent-phase lifetime, the hostile-ingress contract (one request per connection; exact request-line, header, body, connection, and read-time limits; fail-closed framing and path canonicalization before classification; the upstream request rebuilt from validated parts under a fixed client-header allowlist; response headers allowlisted and reframed after buffering; compressed upstream responses refused), and the redacted, size-bounded, write-ahead `gh-audit.jsonl` (an intent record before any credential use, a correlated completion record after carrying the request's redirect decisions as one bounded array) written to a bench-driver-owned sink the container cannot reach and published from it after the agent phase with a summary counting complete, refused, incomplete, and indeterminate requests and reporting any unparseable tail. What the agent may read through it is broad — any author's content — and none of it carries pipeline authority; a bench run has no verdict-parsing driver, so that boundary is exercised in the production driver's tests, not here.
...
- The GitHub Relay: socket lifecycle, the hostile-ingress parser and request/response normalization, policy, budgets and the pre-delivery response gate, per-class redirect rules, write-ahead trusted audit sink and publication, with an injectable upstream (live GitHub.com credential source, or none) (grilling 2026-09-03).
...
- **2026-09-03** — the GitHub Relay joins the Run Contract: `schema_version` 2 — the job dir carries the relay socket; the Context Tree (`input/issue/`, `input/pr/` GitHub-derived parts, `input/deps/`) leaves the job dir and the prompt renderers take the issue body, composed PR body, and the ordered dependency entries (issue number plus state, through the `dependencies` argument replacing the `deps_present` flag) inline; the relay is an exposed entry point — hostile-ingress contract included — with `live` (GitHub.com only) and `none` upstream modes. `bundle_format_version` and `identity_spec_version` unchanged (the new `GITHUB_READ_TOKEN` slot travels as a slot name like every other).
...
| ADR-0057-github-relay | `gh` is the agent's GitHub read surface: a driver-owned per-Run relay socket in the job dir — parsed as hostile ingress — injects a dedicated read-only credential, admits only reads, and replaces the Context Tree; Run Contract `schema_version` 2 with the dependency-entry prompt schema and live/none bench upstreams |
