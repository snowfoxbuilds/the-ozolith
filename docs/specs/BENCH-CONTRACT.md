...
2. Recompute the compiled knowledge tree's pin from bundle bytes with the published tree-hash function (an empty knowledge ref means no knowledge tree may be present), and the baked Agent Policy tree's pin the same way (an empty policy ref means no policy tree may be present; a driverless manifest can never carry a baked policy — ADR-0055).
...
A modified Dockerfile, changed `FROM`, added `RUN`, missing setup instruction, changed knowledge target, modified knowledge or policy byte, unexpected file, or malformed manifest fails verification before Docker is ever invoked.
