...
├── worker/              # workers: node-resident drivers for the two kinds (Issue Worker, PR Worker — see WORKER-MODEL.md), agent harness + run-container Dockerfiles, per-Agent adapters, first-party gate
...
- One private config repo holds all operator content: deployment declarations (Stacks, Worker-Type Definitions — each declaring its kind, Intake, outputs, and prompt, so the pipeline's Implementer and Reviewer are shipped default definitions rather than built-in classes (grilling 2026-09-03; see [WORKER-MODEL.md](WORKER-MODEL.md)) — overlays, secret names — the Config Repo), agent knowledge (skills, subagents, workflows — the Config Repo's `knowledge/` tree; the separate knowledge repo, `knowledge_source`/`knowledge_pin`, and the KNOWLEDGE_GIT_TOKEN slot are retired), operator prompt templates (`prompts/`, content-hashed and pinned at ingest, delivered to nodes in the same Config Distribution as driver code; grilling 2026-09-03), and custom driver code (`drivers/`, delivered to nodes as a hash-pinned config distribution). The "pure data" charter is narrowed for driver code: it is operator content that must not be published in a public checkout; machinery still never lives here.
