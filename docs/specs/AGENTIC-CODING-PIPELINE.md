...
Agent-agnostic framework for staged autonomous development: plan in GitHub issues, implement via Workers executing disposable Runs, review by a dedicated Reviewer actor, merge by a human.
...
- Queue rule: each actor watches one label on one object type. Workers request plan_ready issues through Control Node dispatch; the Reviewer requests pr_ready PRs (without needs_human, under the round budget) through the same dispatch path; the human polls PRs (needs_human). See ADR-0017.
- Transition authority: the human applies initial plan_ready, answers blocked decisions (comment + re-queue), and closes issues. The Control Node applies in_progress + assignee at claim (claim-write-through dispatch; ADR-0017); the Worker applies pr_ready at PR push. The Reviewer owns all post-PR state — deviation:*, risk:*, attempt-N, blocked, needs_human — and, under explicitly delegated authority, the claim strip + plan_ready re-queue that starts the next round. No other transitions exist.
...
- Claim Protocol (ADR-0017): the Worker requests work from the Control Node; the Control Node selects an issue, writes the claim to GitHub itself (assigns the Worker's GitHub login, adds in_progress), and returns the issue in the same response. The Control Node is the single serialized claim-writer; assign-and-verify is deleted. GitHub remains the sole source of coordination truth. A granted claim with no claimed event within the activation window (~60s) is released by the Control Node. Control Node down = new claims and new review rounds pause; in-flight Runs finish and publish.
- Zombie claims (ADR-0016): the Control Node detects claims whose Worker has been silent past the grace period and surfaces them on the dashboard, then waits for the swept evidence bundle to land (boot-time sweep when the driver returns) before releasing the claim and escalating failed + needs_human on the issue with the evidence link. Evidence precedes escalation; there is no automatic re-queue — a node that never returns is a human call from the dashboard. Nodes with 2 consecutive failed Runs are quarantined at dispatch until a human releases them.
- Round budget: 3 review rounds per issue, tracked as attempt-N labels on the PR, incremented by the Reviewer at each revise verdict — never on claim. Round input is the Reviewer's revised plan plus its designated resume commit. Runs that produce no PR consume no rounds — they draw on the separate failed-Run budget (one retry, then escalation; see Run outcomes); Runs lost to a dead Worker escalate through the zombie janitor once swept evidence lands (ADR-0016). The Control Node cross-checks attempt labels against Run and review events and flags mismatches; it does not auto-correct.
...
- A Worker is a long-lived **driver process** on a container-host, bound to one Agent config: a supervised Node Daemon child declared as a process-kind Stack (see [NODE-SUBSTRATE.md](http://node-substrate.md/)). It requests work from the Control Node (ADR-0017) and executes Worker Runs sequentially, one at a time. Workers are not containers — the long-lived-container design is retracted (ADR-0013).
...
- The Reviewer is a separate long-lived actor (own node-resident driver, own GitHub identity, configured with a stronger model than the Worker adapters; no self-grading by construction). It receives pr_ready PRs (without needs_human, under the round budget) through Control Node dispatch (ADR-0017), and owns all post-PR state.
- Verdicts:
...
- **Grilling 2026-07-13**: coordination → GitHub is the sole coordination source of truth; Control Node is advisory (claim pre-filter, zombie-claim janitor, retry auditor). See ADR-0002. [SETTLED, superseded in part 2026-07-17: the claim path is Control Node dispatch. See ADR-0017.]
- **Rejected standalone Dispatcher service**: coordination dissolves into the Claim Protocol on GitHub state. [SUPERSEDED 2026-07-17: claim dispatch centralizes in the Control Node; GitHub remains the record. See ADR-0017.]
...
- **Grilling 2026-07-16**: verdict and run-outcome contracts — any invalid verdict (malformed, failing validation, or a revise on the final round) escalates immediately to blocked + needs_human with no driver-side retry, guarded by an in-session validate-verdict harness job; approve means no revisions needed at all; a concluded no-change Run ships as an empty PR with reasoning and is judged like any PR; a Run producing no PR is a failed Run with a one-retry budget tracked by a run-failed marker comment, then escalation; every Run that reaches a checkout pushes an evidence bundle, including failed Runs. See ADR-0014. [SETTLED, amended 2026-07-17: the failed-Run path is local retry without marker comments; zombie re-queue replaced by evidence-first escalation. See ADR-0016.]
- **Grilling 2026-07-17**: claim dispatch — claims route through the Control Node with claim-write-through (the Control Node writes assignee + in_progress on GitHub before handing the issue to the driver; single serialized claim-writer; assign-and-verify deleted); the Reviewer discovers pr_ready PRs via the same dispatch path; Control Node down = in-flight Runs finish and publish, new claims and review rounds pause. Run generalized: a Run is one ephemeral container lifecycle — Worker Run or Review Run; every Run reaching a checkout pushes evidence. See ADR-0017. [SETTLED]
- **Grilling 2026-07-17 (late)**: failure handling — local retry (driver keeps the claim, one full second Run, uniform budget across failure classes; marker machinery deleted); failed + needs_human on issues (human-only removal; failed overrides plan_ready at dispatch); control-side node quarantine after 2 consecutive failed Runs, human-released; progress telemetry via typed events under a cache-not-archive rule (~10GB transcript budget; evidence bundles are the only durable audit); boot-time evidence sweep (delete only after confirmed push; bundles marked swept); zombie escalation waits for swept evidence — stability and token efficiency over uptime. See ADR-0016. [SETTLED]
