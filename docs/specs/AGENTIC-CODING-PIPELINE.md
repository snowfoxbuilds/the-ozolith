...
- All agent I/O is files. Inputs are materialized into the session's working directory (the repo checkout for Runs; issue, diff, Decisions Section, and mechanical signals as files for reviews). Outputs are files the driver reads: the decisions file (.theozolith/decisions.json) for Runs, the verdict file (verdict.json) for reviews. Both files accept an optional process_issues[] list (pipeline friction observed — bad instructions, missing skills, misleading specs, tooling failures — plus a suggested fix), rendered into the published artifact as a Process issues block: advisory observations about the pipeline, never findings about the change under review; they influence no verdict or label. Harvesting is manual in V1 — read at merge, with fixes flowing through grilling or ordinary issues; auto-aggregation and agent-filed issues are rejected (reserved-Planner territory; the PR is the durable record, not the control DB). Drivers render every published artifact (PR body, verdict comment, labels) from these files; no pipeline state is ever parsed out of model prose.
...
- The **Flight Deck** is the interactive surface: a human-driven agent container declared as a container-kind Stack, running an agent CLI in a discoverably named tmux session — the web terminal's primary attach target. Used for issue drafting and non-decomposable work (cross-cutting refactors, design-in-flux). The Flight Deck holds GitHub credentials under human supervision — a separate machine identity (fine-grained PAT: issues, PRs, contents; no merge permission), never the operator's own PAT, so human-authority transitions (plan_ready, blocked answers, the merge) remain recognizable as human acts and cannot be forged by an injected session; it is not a pipeline actor — it never claims issues and holds no transition authority. Flight Deck session transcripts are the audit trail for human-driven agent work.
### Upgrade path
