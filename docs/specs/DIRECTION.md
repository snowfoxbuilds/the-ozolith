...
- Accepted cost: no mid-run steering or in-band observability. Runs are transactions, not sessions (ADR-0019) — evidence bundles, transcripts, and exit status are the observation surface. In-flight policy, where ever needed, uses the agent's native extension points (e.g. hooks), never a wrapper around the loop.
...
All pipeline state lives in labels, issues, and PRs (ADR-0017). No second source of truth to drift; state is legible in the tool humans already use; Control Node death pauses new claims but never corrupts state.
...
- The primary external output is written design rationale derived from these theses (thin adapters, GitHub-as-truth, credential split, adversarial review), backed by the repo and ADR index.
