...
| docs/adr/ADR-0045-model-effort-baked-into-derived-image.md | Model and reasoning effort are typed worker-type-definition fields materialized by the compiler into the adapter's native config at derived-image build time (never env vars or invocation flags); adapters declare mappable values and a build with an unmappable value fails; the instruction hash covers the materialized config; the Flight Deck bakes a default model with in-session /model as session state; amends ADR-0044/0014 |
